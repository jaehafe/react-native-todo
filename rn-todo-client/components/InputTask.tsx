...
export default function InputTask({ todos, setTodos }: InputTaskProps) {
  const [showEmojies, setShowEmojies] = React.useState(false);
  const [messageBody, setMessageBody] = React.useState('');
  const [fadeAnim] = React.useState(new Animated.Value(0.1));
  ...
  const handleSubmit = async () => {
    if (messageBody === '') {
      ...
      const { data: newTodo } = await axios.post('http://localhost:8080/todos', {
        user_id: 1,
        title: messageBody,
      });
  ...
  const RenderEmoji = ({ emoji }: { emoji: string }) => {
    return (
      <TouchableHighlight
        activeOpacity={1}
